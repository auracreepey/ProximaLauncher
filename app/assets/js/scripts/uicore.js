...
remote.getCurrentWebContents().on('devtools-opened', () => {
    console.log('%cDANGER.', 'color: white; -webkit-text-stroke: 4px #a02d2a; font-size: 60px; font-weight: bold')
    console.log('%cLa console est réservé aux dévelopeurs pour résoudre des problèmes.', 'font-size: 16px')
    console.log('%cSi quelqu\'un t\'a dit de copier/coller quelque chose ici, il y a à peu près 11 chance sur 10 que ce soit une arnaque', 'font-size: 16px')
    console.log('%cA moins que tu sache exactement ce que tu fait, ferme cette fenêtre et demande de l\'aide à aura_creeper.', 'font-size: 16px')
})
        ...
        switch(arg){
            ...
            case 'update-available':
                ...
                if(process.platform === 'darwin'){
                    info.darwindownload = `https://github.com/auracreepey/ProximaLauncher/releases/download/v${info.version}/ProximaLauncher-setup-${info.version}${process.arch === 'arm64' ? '-arm64' : '-x64'}.dmg`
                    showUpdateUI(info)
                }
